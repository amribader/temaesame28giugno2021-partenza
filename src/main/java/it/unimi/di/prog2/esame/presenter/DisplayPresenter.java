...
public class DisplayPresenter implements Observer<List<Articolo>> {
    private final DisplayView view;
    private final Model model;

    public DisplayPresenter(DisplayView view, Model model) {
        this.view = view;
        this.model = model;
    ...
    public void update(Subject<List<Articolo>> subject, List<Articolo> state) {
        ...
        for (Articolo articolo : state) {
           if (articolo.getDaComprare() > 0){
               view.set(i,articolo.getNome()+" "+articolo.getDaComprare());
               i++;
           }
